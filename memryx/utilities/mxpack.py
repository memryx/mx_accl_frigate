...
def _write_list_no_header(wl : list,
    ...
    mtype = 0x00
    if leng == 0:
        # default to FP32 dtype for empty lists
        mtype = 0x50
...
def main():
    ...
    test1 = { "item1": "420",
             "item2": 69 }
    ret1 = MxPackEncode(test1)

    test2 = { "ifmaps": [ {"name": "ifmap", "shape": [224, 224, 3]} ],
              "ofmaps": [ {"name": "big80", "shape": [80, 80, 255]},
                          {"name": "med40", "shape": [40, 40, 255]},
                          {"name": "sml20", "shape": [20, 20, 255]}
                        ],
             "original_fname": "/home/obama/my_awesome_model.onnx",
             "numpy_array": np.ones([2,3,2], dtype=np.float32),
             "target_fps": 42069,
             "negatory": -999999999,
             "empty_list": [],
             "is_Tim_cool_beans": True,
             "the_boolist_bools": [True, False, False, True],
             "thiccc": [1234123412345, 23578923045, -12734823, 33, 12734890],
             "FLOATY": float(-424242),
             "postproc_model": {
                   "file_bytes": ret1,
                   "input_names": [ "big80_in", "med40_in", "sml20_in" ],
                   "min_ORT_version": 17
                 },
             "reorder": [ [0, 1, 2, 3], [1, 3, 0, 2], [3,2,1,0] ]
            }

    print("\nENCODE INPUT:")
    pprint(test2)

    ret2 = MxPackEncode(test2)

    with open('test2.bin', 'wb') as fd:
        fd.write(ret2)

    dec2 = MxPackDecode(ret2)
    print("\nDECODED OUTPUT:")
    pprint(dec2)

    # FIXME: fails because of numpy array ".all()" nonsense
    #if dec2 == test2:
    #    print("\nTEST PASS  :-)\n")
    #    exit(0)
    #else:
