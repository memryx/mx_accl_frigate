...
from memryx.errors import UtilityError
from memryx.utilities.mxpack import MxPackDecode
...
class Dfp:
    ...
    """
    def __init__(self, path_or_bytes: Union[str, Path, bytearray]):
        """
        """
        self._dfp_bytes = None
        if isinstance(path_or_bytes, (str, Path)):
            if not str(path_or_bytes).endswith('.dfp'):
                raise UtilityError(message=f"Input file: {path_or_bytes} is not a .dfp file generated by the NeuralCompiler")
            try:
                with open(path_or_bytes, 'rb') as fh:
                    self._dfp_bytes = BytesIO(fh.read())
            except:
                raise UtilityError(message=f"Error opening file {path_or_bytes}")
        elif type(path_or_bytes) is bytearray:
            self._dfp_bytes = BytesIO(path_or_bytes)
        else:
            raise UtilityError(message=f"dfp argument is not type str or bytearray, it is type={type(path_or_bytes)}")
        ...
        if possible_version_info == 6:
            ...
            raise UtilityError(message=f"Unsupported DFP version: {possible_version_info}, please recompile to the latest version")
    ...
    # close the BytesIO stream when the object is deleted
    def __del__(self):
        if self._dfp_bytes is not None:
            self._dfp_bytes.close()
            del self._dfp_bytes
    ...
    def sub_version(self):
        """
        Returns the DFP sub-version
        """
        return self._metadata.get('dfp_sub_version', 0)
    ...
    def __decode_v6(self, f):
        ...
        meta["dfp_sub_version"]  = dat.get("dfp_sub_version", 0)
        ...
        meta["1p2_compatible"] = dat.get('1p2_compatible', True)
        ...
        for p in dat["inport_info"]:
            ...
            raw_shape = p.get('raw_shape', [])
            if raw_shape:
                raw_shape = [raw_shape[str(key)] for key in range(len(raw_shape))]
            ...
            if p["active"]:
                inport_info_dict[p["port"]] = {
                        ...
                        "shape": p["mxa_shape"],
                        "raw_shape": raw_shape, 
                        "raw_dtype": p.get("raw_dtype", "NA"),
                        "batch": p.get("batch", 0),
                        "shape_shift_info": p.get("shape_shift_info", [])
                    }
            else:
                inport_info_dict[p["port"]] = {"active": False,
                                               "set": 0,
                                               "mxa_id": -1,
                                               "model_index": -1,
                                               "layer_name": "",
                                               "data_type": "float",
                                               "data_range_enabled": False,
                                               "data_range_shift": 0,
                                               "data_range_scale": 0,
                                               "packing_format": "fp32",
                                               "shape": [0, 0, 0, 0],
        ...
        for p in dat["outport_info"]:
            ...
            if p["active"]:
                ...
                outport_info_dict[p["port"]] = {"active": False,
                                                "set": 0,
                                                "mxa_id": -1,
                                                "model_index": -1,
                                                "layer_name": "",
    ...
    def __decode_v5(self, f):
        ...
        if (gen_towers & 0x0F) == 4:
            ...
            raise UtilityError(message=f"Unknown MXA generation {gen_towers & 0x0F} detected!")
...
def pretty_print(meta):
    ...
    if 'dfp_sub_version' in meta:
        print(f"DFP sub-version:   {meta['dfp_sub_version']}")
    ...
    if meta.get('1p2_compatible', True) == False:
        print(f"SDK 1.2 compatible: {meta['1p2_compatible']}")
